package be.oak3.gwt.websocket.client;
...
public final class ChatClient implements EntryPoint, WebSocketListener {
		...
		if (null == webSocket) {
			...
			_connect = new Button("Connect", new ClickHandler() {
				...
				public void onClick(final ClickEvent event) {
					...
					webSocket.connect("ws://localhost:8081/echo");
				}
			});
	...
	private void send(final WebSocket webSocket, final String message) {
		webSocket.send(message);
	}

	public void onMessage(@Nonnull final WebSocket webSocket,
			@Nonnull final ArrayBuffer data) {
	...
	public void onClose(@Nonnull final WebSocket webSocket,
			final boolean wasClean, final int code,
	...
	private void logStatus(@Nonnull final String section,
		...
		final String suffix = !webSocket.isConnected() ? "" : "URL:"
				+ webSocket.getURL() + "\n" + "BinaryType:"
